...
class WXBot:
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            ...
            mp3_data = self.session.get(msg_content['data']).content
            vodata = self.convert_mp3_to_wav(mp3_data)
            msg_content['vlen'] = len(vodata)
            msg_content['voice'] = base64.b64encode(vodata)
            if self.DEBUG:
